...
const locationHelpers_1 = require("../../cucumber/formatter/locationHelpers");
...
class JsonReporter extends base_1.default {
    ...
    getStepData({ isBeforeHook, gherkinStepMap, pickleStepMap, testStep, testStepAttachments, testStepResult, }) {
        ...
        if ((0, valueChecker_1.doesHaveValue)(testStep.stepDefinitionIds) && testStep.stepDefinitionIds.length === 1) {
            const stepDefinition = this.supportCodeLibrary.stepDefinitions.find((s) => s.id === testStep.stepDefinitionIds?.[0]);
            if ((0, valueChecker_1.doesHaveValue)(stepDefinition)) {
                data.match = { location: (0, locationHelpers_1.formatLocation)(stepDefinition) };
            }
        }
        ...
        if (allowedAttachments && allowedAttachments.length > 0) {
            data.embeddings = allowedAttachments.map((attachment) => ({
                data: attachment.contentEncoding === messages.AttachmentContentEncoding.IDENTITY
